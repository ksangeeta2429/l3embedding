...
def get_subset_split(annotation_data, split_path=None):
    ...
    # New annotation data has column 'new_split' which should be used instead of 'split'
    data = annotation_data[['new_split', 'sensor_id', 'audio_filename']].drop_duplicates().sort_values('audio_filename')
    ...
    for idx, (_, row) in enumerate(data.iterrows()):
        if not custom_split:
            ...
            elif row['new_split'] == 'validate':
                valid_idxs.append(idx)
            elif row['new_split'] == 'test':
            ...
            if row['sensor_id'] in custom_split['train']:
                ...
                valid_idxs.append(idx)
            elif row['sensor_id'] in custom_split['test']:
                test_idxs.append(idx)
...
def train_framewise(annotation_path, taxonomy_path, emb_dir, output_dir, cls,
    ...
    # Changed coarse_target_labels for binary classifier
    ...
    fine_target_list = get_file_targets(annotation_data, file_list, full_fine_target_labels)
    coarse_target_list = get_file_targets(annotation_data, file_list, coarse_target_labels)
    ...
    if out_all:    
        ...
        results = results['train'] + results['valid'] + results['test']
        new_file_list = np.array(file_list)[train_file_idxs].tolist() 
        new_file_list += np.array(file_list)[valid_file_idxs].tolist() 
        new_file_list += np.array(file_list)[test_file_idxs].tolist()

        generate_output_file(results, [], output_dir, file_list,
                             annotation_data, aggregation_type, label_mode, 
                             taxonomy, coarse_target_labels
                            )
    else:
...
def train_mil(annotation_path, taxonomy_path, emb_dir, output_dir, clss, label_mode="coarse",
    ...
    # Uses new annotation data created by ust_splits_with_sgrp.ipynb
    file_list = annotation_data.sort_values('audio_filename')['audio_filename'].unique().tolist()
    num_sensors = len(annotation_data.sort_values('sensor_id')['sensor_id'].unique().tolist())
    ...
    # Changing coarse_target_labels to get only one target label for positive class
    cls_dict = {k: v for k, v in taxonomy['coarse'].items() if v == clss}
    coarse_target_labels = ["_".join([str(k), v])
                                for k, v in cls_dict.items()]

    print("* Preparing training data.")

    # Including file_list in case we want to give a subset of files from annotation
    fine_target_list = get_file_targets(annotation_data, file_list, full_fine_target_labels)
    coarse_target_list = get_file_targets(annotation_data, file_list, coarse_target_labels)
    train_file_idxs, valid_file_idxs, test_file_idxs = get_subset_split(annotation_data, split_path=split_path)
    ...
    if out_all:
        ...
        generate_output_file(results, [], output_dir, new_file_list,
                             annotation_data, "", label_mode, taxonomy, coarse_target_labels)
    else:
...
def generate_output_file(y_pred, test_file_idxs, results_dir, file_list,
                         annotation_data, aggregation_type, label_mode, 
                         taxonomy, coarse_target_labels, only_coarse=True):
    """
    ...
    annotation = annotation_data.sort_values('audio_filename')[['new_split', 'sensor_id', 'grp_id', 'audio_filename']].drop_duplicates()
    annotation = annotation[annotation['audio_filename'].isin(test_file_list)]
    split_list = annotation['new_split'].tolist()
    grp_list = annotation['grp_id'].tolist()
    ...
    with open(output_path, 'w') as f:
        ...
        # Write fields
        if only_coarse:
            fields = ["new_split", "sensor_id", "grp_id", "audio_filename"] + coarse_target_labels
        else:
            fields = ["new_split", "sensor_id", "grp_id", "audio_filename"] + full_fine_target_labels + coarse_target_labels
        csvwriter.writerow(fields)

        # Write results for each file to CSV
        for split, sid, gid, filename, y in zip(split_list, sensor_list, grp_list, test_file_list, y_pred):
            row = [split, sid, gid, filename]
...
if __name__ == '__main__':
    ...
    parser.add_argument("clss", type=str)
